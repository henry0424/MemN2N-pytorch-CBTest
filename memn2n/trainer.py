...
class Trainer():
    def __init__(self, config):
        ...
        if 'bAbI' in config.dataset_dir:
            ...
            self.train_data = CBTestDataset(config.dataset_dir, config.word_type, perc_dict=config.perc_dict)
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            ...
            if True:
                ...
                #print("Epoch: ", epoch+1, loss, train_acc, test_acc)
                print("Epoch: ", epoch+1, loss, test_acc)
                torch.save(self.mem_n2n.state_dict(), config.dataset_dir + '_epoch_' + str(epoch+1))
    ...
    def _train_single_epoch(self, epoch):
        ...
        for step, (story, query, answer, options) in enumerate(self.train_loader):
            story = Variable(story)
            query = Variable(query)
            answer = Variable(answer)
            options = Variable(options)
            ...
            self.opt.zero_grad()
            loss = self.ce_fn(self.mem_n2n(story, query, options)[0], answer)
            loss.backward()
            ...
            if (step+1)%100 == 0:
                test_acc = self.evaluate("test")
                print("Step: ", step+1, ' of ', int(np.ceil(self.train_data.__len__()/self.config.batch_size)), ' loss: ', round(loss.item(), 3), ' test accuracy: ', round(test_acc, 3))
